...
export default function Navbar() {
  ...
  return (
    <>
      ...
      {/* Bottom Navigation */}
      <nav style={{ background: 'var(--color-surface)', borderTop: '1px solid var(--color-border)' }}
        className="fixed bottom-0 left-0 right-0 z-50 px-2 py-2">
        <div className="flex items-center justify-around max-w-lg mx-auto">
          {navItems.map((item) => {
            const isActive = location.pathname === item.path;
            return (
              <NavLink key={item.path} to={item.path} className="flex flex-col items-center gap-1 px-2 py-1 rounded-xl transition-all relative">
                {isActive && (
                  <motion.div
                ...
                <span className="text-xs font-medium relative z-10 whitespace-nowrap"
                  style={{ color: isActive ? 'var(--color-gold)' : 'var(--color-text-muted)', fontSize: '9px' }}>
                  {t(item.key)}
                </span>
              </NavLink>
            );
